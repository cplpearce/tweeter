...
const dateDayCalc = (date) => {
  ...
  const daysSince = Math.floor(Math.abs(new Date(date) - Date.now()) / (1000 * 3600 * 24));
  let returnDays;
  if (daysSince > 1) {returnDays = `${daysSince} Days Ago`}
  else if (daysSince === 1) {returnDays = `${daysSince} Day Ago`}
  else {returnDays = 'Today'};
  return returnDays;
}
  ...
  $( '#new-tweet-form' ).submit((event) => {
    ...
    // then post the tweet via ajax
      ...
      $( '#tweet-text' ).fadeTo(500, 0, () => {
        ...
        $( '#tweet-text' ).val('').blur() && $( ".counter" ).html('140') && $( '#tweet-text' ).fadeTo(500, 1);
